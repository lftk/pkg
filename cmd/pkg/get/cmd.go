...
var (
	...
	Command = cli.Command{
		...
		Action: func(c *cli.Context) error {
			return fetchPackages(c.Args())
		},
	}
)

func fetchPackages(paths []string) (err error) {
	var dir string
	if local {
		dir, err = os.Getwd()
		if err != nil {
	...
	for _, path := range paths {
		err = fetchPackage(dir, path, dones)
		if err != nil {
			return
		}
	}
...
func fetchPackage(dir, path string, dones map[string]string) (err error) {
	logVerbose(path)
	...
	logVerbose("  [package]%s", pkg)
	if ver != "" {
		logVerbose("  [version]%s", ver)
	}
	...
	logVerbose("  [base]%s", base)
	logVerbose("  [repository]%s", repo)

	sha, ok := dones[base]
	if ok {
		logVerbose("  [revision]%s", sha)
		return
	}
